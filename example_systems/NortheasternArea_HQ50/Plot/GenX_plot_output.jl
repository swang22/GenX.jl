...
plot_cap_mix(fill_gendf_zero(Exist_agg_cap_df), ordered_tech, color_map,  "Existing Generation Capacity Mix at 2022")
plot_cap_mix(fill_gendf_zero(End_agg_cap_df), ordered_tech, color_map,  "Optimized Generation Capacity Mix at 2050")
...
#hours=1:168
hours=3625:3792#3625:3792#3776:3943
#=
...
function plot_power_output(data::Dict, ordered_tech_power::Vector,ordered_es_tech ::Vector, color_map::Dict,hours::UnitRange)
    ...
    data= [[if (isempty(agg_zone_data[agg_zone_data[!,:Technology] .==tech,:])) filter!(!=(tech), ordered_es_tech )
    ...
    return plot(data, 
                ...
                yaxis_range=[-50000,1.1*max_y_cap],
                showlegend=true,
                barmode="stack")
    )
end
...
agg_gen_data_byzone = combine(groupby(Output_power, [:Zone, :Technology]), names(Output_power, r"AnnualSum") .=> sum)
ordered_gen_tech = ["Coal","Nuc","NGCC","NGCC_CCS","NGCT","Hydro_res","Hydro_ror","Hydro_pump","Bio","BECCS","WindOn","WindOff","SolarPV","Battery","Other_peaker"]

#Fill the missing ones:
function fill_genpowerdf_zero(df)
    combins= DataFrame(Zone = repeat(unique(df[:,:Zone]), inner = length(unique(df[:,:Technology]))),
               Technology = vec(repeat(unique(df[:,:Technology]), outer = length(unique(df[:,:Zone])))))

    combins.Capacity = Array{Union{Missing,Float64}}(undef, size(combins)[1])
    rename!(combins, :Capacity => Symbol("AnnualSum_sum"))

    df_combined = leftjoin(combins, df, on = [:Zone, :Technology], makeunique=true)
    df_combined[:,"AnnualSum_sum"] = coalesce.(df_combined[:,"AnnualSum_sum_1"], 0)
    #drop
    select!(df_combined, Not(Symbol("AnnualSum_sum_1")))
    return df_combined
end

function plot_gen_mix(df::DataFrame, ordered_tech::Vector, color_map::Dict,tt::String)
    agg_df = combine(groupby(df, [:Zone]),Symbol("AnnualSum_sum") => sum)
    rename!(agg_df, Symbol("AnnualSum_sum_sum") => "Gen")
    max_zone_cap = maximum(agg_df.Gen)

    return plot([bar(x=sort(unique(df[:,:Zone])), y= sort(filter(row -> row.Technology == ordered_tech[i],df), :Zone)[:,"AnnualSum_sum"]/1000000, marker_color=color_map[ordered_tech[i]], name=ordered_tech[i] ) for i in 1:size(ordered_tech)[1]], 
    Layout(title=tt, barmode="stack", 
    xaxis_categoryorder="category ascending", xaxis_title_text="Regions",
    yaxis_title_text="Generatoin (TWh)", 
    #yaxis_range=[0,1.1*max_zone_cap]
    yaxis_range=[0,400]
    ))
end

plot_gen_mix(fill_genpowerdf_zero(agg_gen_data_byzone), ordered_gen_tech, color_map,  "Power Generation Mix at 2050" )
#plot power generation--------------------------------------------------------#
