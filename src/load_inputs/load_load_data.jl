...
function load_load_data!(setup::Dict, path::AbstractString, inputs::Dict)
    ...
    if setup["NetImport"] == 1
        NIfilename = "NI_data.csv"
        ni_in = load_dataframe(joinpath(my_dir, NIfilename))
    end
    ...
    if setup["NetImport"] == 1
        NIstart = findall(s -> s == "NI_MW_z1", names(ni_in))[1] 
        inputs["pNI"] =Matrix(ni_in[1:T, NIstart:NIstart+Z-1]) / scale_factor  # convert to GW
    end
