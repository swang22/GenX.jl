...
function load_generators_data!(setup::Dict, path::AbstractString, inputs_gen::Dict, fuel_costs::Dict, fuel_CO2::Dict)
	...
	#inputs_gen["HYDRO_RES"] = gen_in[(gen_in[!,:HYDRO].==1) .& (gen_in.HYDRO_WD.!=1),:R_ID]
	inputs_gen["HYDRO"] = gen_in[(gen_in[!,:HYDRO].==1),:R_ID]
	
	if setup["WaterDynamic"] == 1
		inputs_gen["HYDRO_RES"] = gen_in[(gen_in[!,:HYDRO].==1) .&(gen_in[!,:HYDRO_RoR].!=1),:R_ID]
		inputs_gen["HYDRO_WD"] = gen_in[(gen_in[!,:HYDRO].==1) .& (gen_in.HYDRO_WD.==1),:R_ID]
		inputs_gen["HYDRO_RES_notWD"] = gen_in[(gen_in[!,:HYDRO].==1) .& (gen_in.HYDRO_WD.!=1) .&(gen_in[!,:HYDRO_RoR].!=1),:R_ID]
		inputs_gen["HYDRO_RoR_WD"] = gen_in[(gen_in[!,:HYDRO_RoR].==1) .& (gen_in.HYDRO_WD.==1),:R_ID]
		#If RoR not modeded with WD, then modeled as VRE 
		inputs_gen["HYDRO_RoR_notWD"] = gen_in[(gen_in[!,:HYDRO_RoR].==1) .& (gen_in.HYDRO_WD.!=1),:R_ID]
		inputs_gen["pPF"] = gen_in[!,"Hydro_productivity_factor_MW_per_m3_per_s"]
	else
		inputs_gen["HYDRO_RES"] = gen_in[(gen_in[!,:HYDRO].==1),:R_ID]
	end 
	
	# Set of reservoir hydro resources modeled with water dynamic
	#Read water dynamic modeling inputs
	try	
		inputs_gen["HYDRO_RoR"] = gen_in[(gen_in[!,:HYDRO].==1) .& (gen_in.HYDRO_RoR.==1),:R_ID]
		inputs_gen["HYDRO_LRES"] = gen_in[(gen_in[!,:HYDRO].==1) .& (gen_in.HYDRO_LRES.==1),:R_ID]
		inputs_gen["HYDRO_SRES"] = gen_in[(gen_in[!,:HYDRO].==1) .& (gen_in.HYDRO_SRES.==1),:R_ID]
	catch
		println("Generators_data.csv may not contain water dynamic parameters")
	end
