...
function curtailable_variable_renewable!(EP::Model, inputs::Dict, setup::Dict)
	...
	if setup["WaterDynamic"]!=1
		try 	
			HYDRO_RoR = inputs["HYDRO_RoR"]
			#Exclude the RoR, RoR is treated as VRE in general case
			VRE = union(VRE,HYDRO_RoR)
			#println(VRE)
		catch
			println("'HYDRO_RoR' is not flagged in Generators_data.csv")
		end
	else
	...
	for y in VRE_POWER_OUT
		...
		if Reserves == 1
			curtailable_variable_renewable_reserves!(EP, inputs, setup)
		else
...
```
"""
function curtailable_variable_renewable_reserves!(EP::Model, inputs::Dict, setup::Dict)
