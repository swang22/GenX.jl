...
function hydro_res!(EP::Model, inputs::Dict, setup::Dict)
	...
	#HYDRO_RoR = dfGen[(dfGen[!,:HYDRO].==1) .& (dfGen.HYDRO_RoR.==1),:R_ID]
	if setup["WaterDynamic"]!=1	
		HYDRO_RoR = dfGen[(dfGen[!,:HYDRO].==1) .& (dfGen.HYDRO_RoR.==1),:R_ID]
	else
		HYDRO_RoR = inputs["HYDRO_RoR_notWD"]
	end
	#print(setdiff(HYDRO_RES,HYDRO_RoR))
	HYDRO_RES_KNOWN_CAP = inputs["HYDRO_RES_KNOWN_CAP"] # Reservoir hydro resources modeled with unknown reservoir energy capacity
	print(HYDRO_RES)
	print(HYDRO_RoR)
	print(HYDRO_RES_KNOWN_CAP)
	#if setup["WaterDynamic"] != 1
		#Exclude the RoR, as RoR is treated as VRE in general case
	#	HYDRO_RES = inputs["HYDRO_RES"] # Set of all reservoir hydro resources, used for common constraints
	#	HYDRO_RES_KNOWN_CAP = inputs["HYDRO_RES_KNOWN_CAP"] # Reservoir hydro resources modeled with unknown reservoir energy capacity
	
	#elseif setup["WaterDynamic"] == 1
	#	HYDRO_RES_notWD = inputs["HYDRO_RES_notWD"] 
	#	HYDRO_RES_KNOWN_CAP = intersect(HYDRO_RES,inputs["HYDRO_RES_KNOWN_CAP"]) # Reservoir hydro resources modeled with unknown reservoir energy capacity
	...
	if setup["WaterDynamic"] != 1
		...
		@variable(EP, vSPILL[y in HYDRO_RES, t=1:T] >= 0)
			...
			# Reservoir hydro storage level of resource "y" at hour "t" [MWh] on zone "z" - unbounded
			@variable(EP, vS_HYDRO[y in union(HYDRO_RES,HYDRO_RoR), t=1:T] >= 0);

			# Hydro reservoir overflow (water spill) variable
			@variable(EP, vSPILL[y in union(HYDRO_RES,HYDRO_RoR), t=1:T] >= 0)
	end
	...
	if setup["WaterDynamic"]!=1	
		HYDRO_RoR = dfGen[(dfGen[!,:HYDRO].==1) .& (dfGen.HYDRO_RoR.==1),:R_ID]
		#print(HYDRO_RoR)
		#Exclude the RoR, RoR is treated as VRE in general case
		@expression(EP, ePowerBalanceHydroRes[t=1:T, z=1:Z],
			sum(EP[:vP][y,t] for y in intersect(setdiff(HYDRO_RES,HYDRO_RoR), dfGen[(dfGen[!,:Zone].==z),:R_ID])))
	...
	#	HYDRO_RES_notWD = inputs["HYDRO_RES_notWD"] 
	#	@expression(EP, ePowerBalanceHydroRes[t=1:T, z=1:Z],
	#		sum(EP[:vP][y,t] for y in intersect(HYDRO_RES_notWD, dfGen[(dfGen[!,:Zone].==z),:R_ID])))
	#end
	...
	### Constraints commmon to all reservoir hydro WiTHOUT 	WD (y in set HYDRO_RES) ###
	if setup["WaterDynamic"] != 1
		...
		@constraints(EP, begin
			# Constraints for reservoir hydro
			cHydroReservoir[y in HYDRO_RES_notWD, t in 1:T], EP[:vS_HYDRO][y,t] == (EP[:vS_HYDRO][y, hoursbefore(p,t,1)]
			- (1/dfGen[y,:Eff_Down]*EP[:vP][y,t]) - vSPILL[y,t] + inputs["pP_Max"][y,t]*EP[:eTotalCap][y])
		end)
	end


	### Constraints commmon to all reservoir hydro (y in set HYDRO) ###
	@constraints(EP, begin
	### NOTE: time coupling constraints in this block do not apply to first hour in each sample period;
		...
		cHydroMaxOutflow[y in setdiff(HYDRO_RES,HYDRO_RoR), t in 1:T], EP[:vP][y,t] <= EP[:vS_HYDRO][y, hoursbefore(p,t,1)]
		cHydroRoRStore[y in HYDRO_RoR, t in 1:T], 0 == EP[:vS_HYDRO][y, t]

		#Intial hydro level limit
		#cHydroIntialLimit[y in setdiff(HYDRO_RES,HYDRO_RoR),t in [1]], EP[:vS_HYDRO][y,t] == dfGen[y,:Hydro_level]*EP[:eTotalCap][y]
		#cHydroEndLimit[y in setdiff(HYDRO_RES,HYDRO_RoR),t in [T]], EP[:vS_HYDRO][y,t] ==  dfGen[y,:Hydro_level]*EP[:eTotalCap][y]
		#cHydroEndLimit[y in setdiff(HYDRO_RES,HYDRO_RoR),t in [T]], EP[:vS_HYDRO][y,t] ==  EP[:vS_HYDRO][y,1]
	end)
	...
	@constraints(EP, begin 
		cHydroMaxEnergy[y in HYDRO_RES_KNOWN_CAP, t in 1:T], EP[:vS_HYDRO][y,t] <= dfGen[y,:Hydro_Energy_to_Power_Ratio]*EP[:eTotalCap][y]
		cHydroIntialLimit[y in HYDRO_RES_KNOWN_CAP,t in [1]], EP[:vS_HYDRO][y,t] == dfGen[y,:Hydro_level]*dfGen[y,:Hydro_Energy_to_Power_Ratio]*EP[:eTotalCap][y]
		cHydroEndLimit[y in HYDRO_RES_KNOWN_CAP,t in [T]], EP[:vS_HYDRO][y,t] ==  EP[:vS_HYDRO][y,1]
		cHydroMinLimit[y in HYDRO_RES_KNOWN_CAP,t in 1:T], EP[:vS_HYDRO][y,t] >= dfGen[y,:Hydro_min_level]*dfGen[y,:Hydro_Energy_to_Power_Ratio]*EP[:eTotalCap][y]
		clvlRampUp[y in HYDRO_RES_KNOWN_CAP, t in 1:T], EP[:vP][y,t] + regulation_term[y,t] + reserves_term[y,t] - EP[:vP][y, hoursbefore(p,t,1)] <= dfGen[y,:SL_Ramp_Up_Percentage]*dfGen[y,:Hydro_Energy_to_Power_Ratio]*EP[:eTotalCap][y]
        clvlRampDown[y in HYDRO_RES_KNOWN_CAP, t in 1:T], EP[:vP][y, hoursbefore(p,t,1)] - EP[:vP][y,t] - regulation_term[y,t] + reserves_term[y, hoursbefore(p,t,1)] <= dfGen[y,:SL_Ramp_Dn_Percentage]*dfGen[y,:Hydro_Energy_to_Power_Ratio]*EP[:eTotalCap][y]
	end)
	for y in HYDRO_RES_KNOWN_CAP
		print(dfGen[y,:Hydro_min_level]*dfGen[y,:Hydro_Energy_to_Power_Ratio]*EP[:eTotalCap][y])
	end
...
function hydro_res_reserves!(EP::Model, inputs::Dict)
	...
	HYDRO_RES = inputs["HYDRO"]#try all hydros
	...
	#println([HYDRO_RES_REG_ONLY,HYDRO_RES_RSV_ONLY])
