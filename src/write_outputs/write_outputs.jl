...
function write_outputs(EP::Model, path::AbstractString, setup::Dict, inputs::Dict)
	...
	#Water dymamic related
	if setup["WaterDynamic"] == 1	
		println("Writing Water Dynamic")
		dfW_dis = write_water_discharge(path, inputs, setup, EP)
		dfW_spill = write_water_spill(path, inputs, setup, EP)
		dfW_bypass = write_water_bypass(path, inputs, setup, EP)
		dfW_balance = write_water_balance(path, inputs, setup, EP)
		if setup["Reserves"] == 1
			dfW_hprbalance = write_hydro_power_reserve_balance(path, inputs, setup, EP)
		end
	end
