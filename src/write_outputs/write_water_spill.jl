@doc raw"""
write_water_spill(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)

Function for writing the different values of water generated by the different hydro technologies in operation.
"""
function write_water_spill(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	dfGen = inputs["dfGen"]
	...
	if setup["ParameterScale"] == 1
		w_spill *= ModelScalingFactor
	end
	scale_factor = setup["ParameterScale"] == 1 ? ModelScalingFactor : 1
	if !isempty(WD_All)
	    w_spill[WD_All, :] = value.(EP[:vW_SPILL][WD_All, :]) * scale_factor
	end
	dfHydroPower.AnnualSum .= w_spill * inputs["omega"]
	dfHydroPower = hcat(dfHydroPower, DataFrame(w_spill, :auto))

	auxNew_Names=[Symbol("Resource");Symbol("Zone");Symbol("AnnualSum");[Symbol("t$t") for t in 1:T]]
	rename!(dfHydroPower,auxNew_Names)

	total = DataFrame(["Total" 0 sum(dfHydroPower[!,:AnnualSum]) fill(0.0, (1,T))], :auto)
	total[:, 4:T+3] .= sum(w_spill, dims = 1)

	rename!(total,auxNew_Names)
	dfHydroPower = vcat(dfHydroPower, total)
	CSV.write(joinpath(path, "water_spill.csv"), dftranspose(dfHydroPower, false), writeheader=false)
	return dfHydroPower
end
